...
import { Textarea, Flex, Heading, Badge, Input, Select } from '@chakra-ui/react'
...
export const ChatPanel = () => {
    ...
    const Load = (finished) => {
        WebHelper.get(`battlemap/getchat`, x => {
            setItems(x);
            finished();
        }, finished);
    }
    ...
    const CreateChatMessage = (x, nextX) => {
        ...
        return (
            <>
                <DListItem isSelected={player !== undefined && player.id === game.CurrentPlayerId}>
                    {elementsList}
                </DListItem>
                {!showLabel ? (<></>) : (
                    <Flex marginLeft={'25px'} direction='row' alignItems={'center'} justifyItems={'center'} verticalAlign={'middle'}>
    ...
    return (
        <Loadable OnLoad={Load}>
            ...
            <BasePanel>
                <Flex grow={1} direction="column-reverse" overflowY="auto" overflowX="hidden" onScroll={(e) => { if (e.currentTarget.scrollTop + e.currentTarget.scrollHeight - e.currentTarget.clientHeight < 3 && !reachedTop) { AppendChat() } }} scrollBehavior="">
                    {items.map((a, i) => CreateChatMessage(a, items[i + 1]))}
                </Flex>
                <Flex direction="column" style={{ display: searchOptions ? 'block' : 'none' }}>
                    <DContainer>
                        <DLabel>Search By: </DLabel>
                        <Input value={filter} onChange={(e) => { setFilter(e.target.value); setReloadChat(true); }} />
                        <DLabel>From: </DLabel>
                        <Select onChange={(e) => { setFrom(e.target.value); setReloadChat(true); }}>
                            <option value={null}>Any</option>
                            {game.players.map(x => (<option value={x.id}>{x.name}</option>))}
                        </Select>
                    </DContainer>
                </Flex>
                <Flex direction="row">
                    <DListItemButton icon={FaSearch} onClick={() => setSearchOptions(!searchOptions)} />
                    <Textarea onKeyDown={(e) => { if (e.key == "Enter") { e.preventDefault(); HandleSend(); } }} onChange={(t) => setMessage(t.target.value)} value={message} resize={'none'}></Textarea>
