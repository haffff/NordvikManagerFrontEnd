...
import { Flex, FormLabel, HStack, Heading, Input } from "@chakra-ui/react";
...
export const DebugConsolePanel = ({ state, actionName }) => {
  ...
  const PresentVariable = (x) => {
    return (
      <div>
        {x} -{" "}
        {typeof lastMessage?.data?.Variables[x] == "object" ? (
          <DListItemButton
            icon={FaEye}
            onClick={() => showDetails(lastMessage?.data?.Variables[x], x)}
          />
        ) : (
          lastMessage?.data?.Variables[x]
        )}
      </div>
    );
  };
      ...
      <DContainer>
        <DButtonHorizontalContainer>
          <Input
            ...
            value={_actionName}
            onChange={(x) => setActionName(x.target.value)}
          />
          <DListItemButton
            label="Run Action"
            icon={FaPlay}
      ...
      {isToggled ? (
        <DContainer>
          <DButtonHorizontalContainer>
            ...
            {lastMessage ? (
              <DLabel>
                {lastMessage?.data?.Step?.Type} - {lastMessage?.data?.Message}
              </DLabel>
            ) : (
              <></>
            )}
      ...
      <Flex overflowY={"auto"} height={"100%"}>
        <DContainer width={"550px"} maxWidth={"700px"}>
          <DList mainComponent={true}>
            <Flex>
              <Heading size={"xs"}>Incoming Messages</Heading>
              <DListItemsButtonContainer>
                <DListItemButton
                  label="Clear"
                  icon={FaBroom}
                  onClick={() => {
                    incomingMessagesRef.current = [];
                    forceUpdate();
                  }}
                />
              </DListItemsButtonContainer>
            </Flex>
            {incomingMessagesRef.current.map((x) => (
              <div>
                {x.date} - {x.command}{" "}
                <DListItemButton icon={FaEye} onClick={() => showDetails(x)} />
              </div>
            ))}
          </DList>
        </DContainer>
        <DContainer width={"550px"}>
          <DList mainComponent={true}>
            <Flex>
              <Heading size={"xs"}>Debug Messages</Heading>
              <DListItemsButtonContainer>
                <DListItemButton
                  label="Clear"
                  icon={FaBroom}
            ...
            {debugMessagesRef.current.map((x) => (
              <div>
        ...
        <DContainer width={"550px"}>
          <DList mainComponent={true}>
            ...
            {lastMessage?.data?.Variables ? (
              Object.keys(lastMessage?.data?.Variables).map((x) =>
                PresentVariable(x)
              )
            ) : (
              <></>
