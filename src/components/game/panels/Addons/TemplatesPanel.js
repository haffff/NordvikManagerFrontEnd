...
export const TemplatesPanel = ({ gameDataRef }) => {
  ...
  React.useEffect(() => {
    const getData = async () => {
      let gameId = await ClientMediator.sendCommandAsync("Game","GetGameId");
      let templates = await WebHelper.getAsync(
        ...
        <Stack width={300}>
          <DList mainComponent={true} withAddButton handleAdd={AddCard}>
            {panelsRef.current?.map((x) => (
              <DListItem
                isSelected={selectedPanel?.id === x.id}
                key={x.id}
                onClick={() => {
                  setSelectedPanel(x);
                }}
          ...
          <DContainer width={"100%"} height={"100%"}>
            {selectedPanel ? (
              <SettingsPanelWithPropertySettings
                ...
                gameDataManagerRef={gameDataRef}
                editableKeyLabelDict={editableDict}
                dto={selectedPanel}
                onSave={(dto) => {
                  WebSocketManagerInstance.Send({
                    command: "template_update",
                    data: { ...selectedPanel, ...dto },
                  });
                }}
              />
            ) : (
