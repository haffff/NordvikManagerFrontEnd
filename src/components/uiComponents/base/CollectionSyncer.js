...
export const CollectionSyncer = ({ 
    ...
    const HandleMessage = (response) => {
        ...
        switch (response.command) {
            ...
            case `${updateCommand}`:
                let updateIndex = collection.findIndex(x => x.id === response.data.id);
                if (updateIndex !== -1) {
                    
                    if(incrementalUpdate)
                    {
                        collection[updateIndex] = {...collection[updateIndex], ...response.data};
                    }
                    else
                    {
            ...
            case `${selectItemCommand}`:
                setSelectedItem(collection.find(x => x.id == response.data || x.id == response.data?.id));
