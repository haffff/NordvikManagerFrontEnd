...
export const Battlemap = ({ withID, keyboardEventsManagerRef }) => {
  ...
  const DrawGrid = () => {
    let objects = editor.canvas.getObjects('group').filter(x => x.name === ".grid");
    if (objects.length > 0) {
    ...
    if (map.gridVisible) {
      var grid = GridFactoryInstance.DrawGrid(map.gridSize, [map.width, map.height]);
  ...
  //Put in loadable
  if (!battleMapModel) {
    WebHelper.get(`battlemap/getbattlemap?id=${withID}`, (resp) => {
  ...
  if (battleMapObjectRef !== undefined && battleMapObjectRef.current.Panel !== ctx.layoutContent.panel) {
    ...
    if (ev.dataTransfer.items) {
      ...
      [...ev.dataTransfer.items].forEach((item, i) => {
        ...
        if (item.kind === "string") {
          ...
          {
            console.log(dragObj);
            fabric.Image.fromURL(`${WebHelper.ApiAddress}/Materials/Resource?id=${dragObj.id}`, (img) => {
              const obj = img;
              if(!obj.width)
              {
                return;
              ...
              var cmd = CommandFactory.CreateAddCommand({ object: JSON.stringify(obj), properties: [], mapId: map.id, layer: selectedLayer });
        ...
        if (item.kind === "file") {
            WebHelper.postMaterial(item.getAsFile(), `${mapRef.current.name}/images`, (result) => {
              fabric.Image.fromURL(`${WebHelper.ImageAddress}${result.id}`, (img) => {
                const obj = img;
                obj.left = coords.x + (10 * i);
                obj.top = coords.y + (10 * i);
                var cmd = CommandFactory.CreateAddCommand({ object: JSON.stringify(obj), properties: [], mapId: map.id, layer: selectedLayer });
  ...
  return (
    <Flex
      ...
      flexGrow={1} direction={'row'}
      height='100%'
      width='100%'
      className='content'
      tabIndex={-1}
      onDrop={(e) => { e.preventDefault(); HandleDrop(e) }} grow={1}
      onContextMenu={(e) => { e.preventDefault(); }}>
      <FabricJSCanvas onReady={onReady} />
      <div ref={popupRef} style={{ display: 'none', position: 'absolute', padding: '5px', margin: "10px", backgroundColor: '#353535', border: '1px solid #ddd' }}>
        {popupContent}
      </div>
      <div ref={contextMenuRef} style={{ display: 'none', position: 'absolute' }}>
        <BattleMapContextMenu contextMenuReference={contextMenuRef} loaded={loaded} battleMapId={uuid} canvas={editor?.canvas} />
      </div>
    </Flex>
  )
  ...
  function LoadCanvas() {
    ...
    BattleMapServices.BMService._BMQueryService = BattleMapServices.BMQueryService;
    ...
    if (map.elements.length > 0) {
      ...
      map.elements.forEach(dto => {
        ...
        object.selectable = object.selectablePermission && dto.layer == selectedLayer;
      ...
      try {
        editor.canvas.loadFromJSON({ objects: canvasObjects }, () => {
          DrawGrid();
          editor.canvas._objects.sort((a, b) => (a.layer > b.layer || a.insideLayerIndex > b.insideLayerIndex) ? 1 : -1);

          editor.canvas.getObjects().filter(x => {
            if (!x.properties)
              return false;
            ...
            if (object.tokenUiElements) {
              fabric.util.enlivenObjects(object.tokenUiElements, (e) => {
                e.forEach(element => {
                  ...
                  element.enabled = UtilityHelper.ParseBool(object.properties["show_" + element.name]?.value);
